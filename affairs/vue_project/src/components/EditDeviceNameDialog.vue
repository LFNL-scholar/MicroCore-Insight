...
export default {
  ...
  setup(props, { emit }) {
    const deviceName = ref(props.currentName)
    ...
    const isValid = computed(() => {
      return deviceName.value.trim().length > 0
    })
    ...
    const handleConfirm = async () => {
      ...
      try {
        await emit('confirm', deviceName.value.trim())
